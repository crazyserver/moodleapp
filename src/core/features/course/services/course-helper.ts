...
export class CoreCourseHelperProvider {
    ...
    isSectionStealth(section: CoreCourseWSSection): boolean {
        return section.hiddenbynumsections === 1 || section.id === COURSE_STEALTH_MODULES_SECTION_ID;
    }
    ...
    async calculateSectionStatus(
        ...
        if (section.id === COURSE_ALL_SECTIONS_ID) {
    ...
    async prefetchSections(
        ...
        if (updateAllSections) {
            ...
            if (sections[0].id !== COURSE_ALL_SECTIONS_ID) {
    ...
    protected async prefetchSingleSectionIfNeeded(section: CoreCourseSectionWithStatus, courseId: number): Promise<void> {
        if (section.id === COURSE_ALL_SECTIONS_ID || section.hiddenbynumsections) {
            return;
        }
        ...
        promises.push(CorePromiseUtils.ignoreErrors(
            CoreFilepool.addFilesToQueue(siteId, introFiles, COURSE_COMPONENT, courseId),
        ));
    ...
    async deleteCourseFiles(courseId: number): Promise<void> {
        ...
        await Promise.all([
            ...
            siteId && CoreFilepool.removeFilesByComponent(siteId, COURSE_COMPONENT, courseId),
        ]);
