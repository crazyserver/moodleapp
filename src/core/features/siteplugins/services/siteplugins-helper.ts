...
export class CoreSitePluginsHelperProvider {
    ...
    protected async clearPreviousStyles(
        ...
        const files = await CoreUtils.ignoreErrors(
            CoreFilepool.getFilesByComponent(siteId, CoreSitePluginsProvider.COMPONENT, componentId),
        );

        files?.forEach((file) => {
            if (file.url !== url) {
                // It's not the current file, delete it.
                CoreUtils.ignoreErrors(CoreFilepool.removeFileByUrl(siteId, file.url));
            }
        });
    }
