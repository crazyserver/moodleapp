...
export default class CoreReportBuilderListPage implements OnInit {
    ...
    protected page = 0;
    protected perPage = REPORTS_LIST_LIMIT;
    ...
    async fetchReports(reload = false): Promise<void> {
        ...
        try {
            const pageToLoad = reload ? 0 : Math.ceil(this.reports().length / this.perPage);
            const reports = await CoreReportBuilder.getReports(pageToLoad, this.perPage);
            if (!reload) {
            ...
            this.hasMoreItems.set(reports.length >= this.perPage * this.page);
        } finally {
