...
export class CoreObject {
    ...
    static sortValues<T>(obj: T): T {
        if (typeof obj === 'object' && !Array.isArray(obj)) {
            ...
            const array = this.toArrayOfObjects(obj as Record<string, unknown>, 'name', 'value', false, true);

            return this.toKeyValueMap(array, 'name', 'value') as unknown as T;
        }
    ...
    static toGetParams(object: Record<string, unknown>, removeEmpty: boolean = true): string {
        ...
        const flattened = this.flatten(object);
        let result = '';
        let joinChar = '';
    ...
    static flatten(obj: Record<string, unknown>, useDotNotation?: boolean): Record<string, unknown> {
        ...
        for (const name in obj) {
            ...
            if (typeof value === 'object' && !Array.isArray(value)) {
                const flatObject = this.flatten(value as Record<string, unknown>);
