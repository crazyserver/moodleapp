...
export class CoreMimetypeUtilsProvider {
    ...
    getEmbeddedHtml(file: CoreFileEntry, path?: string): string {
        const filename = CoreFileSingleton.isFileEntry(file) ? (file as FileEntry).name : file.filename;
        const extension = !CoreFileSingleton.isFileEntry(file) && file.mimetype
            ? this.getExtension(file.mimetype)
            : (filename && this.getFileExtension(filename));
        const mimeType = !CoreFileSingleton.isFileEntry(file) && file.mimetype
            ? file.mimetype
        ...
        if (extension && this.canBeEmbedded(extension)) {
            ...
            path = path ?? (CoreFileSingleton.isFileEntry(file) ? CoreFile.getFileEntryURL(file) : CoreFileHelper.getFileUrl(file));
    ...
    getMimetypeDescription(obj: CoreFileEntry | string, capitalise?: boolean): string {
        ...
        if (typeof obj == 'object' && CoreFileSingleton.isFileEntry(obj)) {
