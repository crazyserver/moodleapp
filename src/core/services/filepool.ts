        ...
        try {
            ...
            if (errorObject && errorObject.source === fileUrl) {
                ...
                switch (errorObject.code) {
                    case CoreFileTransferErrorCore.ABORT_ERR:
                        // The transfer was aborted, we will keep the file in queue.
                        break;
                    case CoreFileTransferErrorCore.FILE_NOT_FOUND_ERR: // File wasn't found, most likely a 404, remove from queue.
                    case CoreFileTransferErrorCore.INVALID_URL_ERR: // The URL is invalid, we drop the file from the queue.
                    case CoreFileTransferErrorCore.CONNECTION_ERR: // If there was an HTTP status, then let's remove from the queue.
                    case CoreFileTransferErrorCore.NOT_MODIFIED_ERR: // We have the latest version of the file, HTTP 304 status.
...
const enum CoreFileTransferErrorCore {
