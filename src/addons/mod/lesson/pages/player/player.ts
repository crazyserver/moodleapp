...
import { ADDON_MOD_LESSON_COMPONENT, AddonModLessonPageId } from '../../constants';
...
export class AddonModLessonPlayerPage implements OnInit, OnDestroy, CanLeave {
    ...
    readonly LESSON_EOL = AddonModLessonPageId.EOL;
    ...
    protected async finishRetake(outOfTime?: boolean): Promise<void> {
        ...
        this.logPageLoaded(AddonModLessonPageId.EOL, Translate.instant('addon.mod_lesson.congratulations'));
    }
    ...
    protected async jumpToPage(pageId: number): Promise<void> {
        if (pageId === 0) {
            ...
            return;
        } else if (pageId == AddonModLessonPageId.EOL) {
    ...
    protected async launchRetake(pageId?: number): Promise<void> {
        ...
        if (this.review) {
            ...
            if (finished) {
                ...
                pageId = AddonModLessonPageId.EOL;
            }
        }
    ...
    protected async loadPage(pageId: number): Promise<void> {
        if (pageId == AddonModLessonPageId.EOL) {
            // End of lesson reached.
            return this.finishRetake();
        } else if (!this.lesson) {
            return;
        ...
        if (data.newpageid == AddonModLessonPageId.EOL) {
            // End of lesson reached.
            return this.finishRetake();
        }
