...
import {
    ...
    AddonModScormGradeMethod,
    AddonModScormMode,
    AddonModScormWhatGrade,
...
export class AddonModScormProvider {
    ...
    calculateScormGrade(scorm: AddonModScormScorm, onlineAttempts: Record<number, AddonModScormAttemptGrade>): number {
        ...
        switch (scorm.whatgrade) {
            case AddonModScormWhatGrade.FIRSTATTEMPT:
                return onlineAttempts[1] ? onlineAttempts[1].score : -1;

            case AddonModScormWhatGrade.LASTATTEMPT: {
            ...
            case AddonModScormWhatGrade.HIGHESTATTEMPT: {
            ...
            case AddonModScormWhatGrade.AVERAGEATTEMPT: {
    ...
    determineAttemptAndMode(
        ...
        if (!canSaveTracks) {
            return {
                mode: scorm.hidebrowse ? AddonModScormMode.MODENORMAL : mode,
                attempt,
                newAttempt: false,
            };
        }

        if (mode == AddonModScormMode.MODEBROWSE) {
            if (scorm.hidebrowse) {
                // Prevent Browse mode if hidebrowse is set.
                mode = AddonModScormMode.MODENORMAL;
            } else {
        ...
        if (scorm.forcenewattempt == AddonModScormForceAttempt.SCORM_FORCEATTEMPT_ALWAYS) {
        ...
        if (newAttempt && (!scorm.maxattempt || attempt < scorm.maxattempt)) {
            ...
            if (incomplete) {
                ...
                mode = AddonModScormMode.MODEREVIEW;
            }
        }
    ...
    formatGrade(scorm: AddonModScormScorm, grade: number): string {
        ...
        if (scorm.grademethod !== AddonModScormGradeMethod.GRADESCOES && scorm.maxgrade) {
    ...
    async getAttemptGrade(
        ...
        switch (scorm.grademethod) {
            case AddonModScormGradeMethod.GRADEHIGHEST:
                score = attemptScore.max;
                break;

            case AddonModScormGradeMethod.GRADEAVERAGE:
            ...
            case AddonModScormGradeMethod.GRADESUM:
                score = attemptScore.sum;
                break;

            case AddonModScormGradeMethod.GRADESCOES:
